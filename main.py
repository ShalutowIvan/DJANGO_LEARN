...
#Перейдем в консоль джанго. Сделаем запрос всех полей Home.objects.all(). Будет возвращен список, к нему можно применять срезы. Срезы идут на уровне sql запроса, а не питона как в питоне просто срез списка. Чтобы проверить, импортируем from django.db import connection и сделаем выведем список sql запросов командой connection.queries. Там в конце будет sql запрос с лимитом 5, так как мы сделали срез из 5 элементов. Если сделать срез с 3 до 8, то будет запрос ASC LIMIT 5 OFFSET 3', Начиная с 3 и всего 5 элементов.
