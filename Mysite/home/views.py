...
class RegisterUser(DataMixin, CreateView):
	# form_class = UserCreationForm#это стандартная форма джанго для регистрации пользователей. 
	form_class = RegisterUserForm#мы сделали свою форму и ее тут используем, вместо формы джанго. Пока она будет выглядеть примерно также как стандартная джанго. Перейдем в файл forms.py
	template_name = 'home/register.html'#это ссылка на шаблон. В него получается будут отправляться все данные из джанго для отображения полей. в самом html поля не прописываются. Там еще текст какой тоже из джанго тянется.
	success_url = reverse_lazy('login')#это перенаправление на url адрес при успешной регистрации пользователя. То есть когда пользователь введет все данные и нажем зарегистрироваться или "Регистрация", то перейдем на функцию представления login. Также если ввести что то не так, ошибки будут уже выдаваться сами по себе, так как джанго сам прописывает все эти ошибки. 

    #далее формируем контекст
	def get_context_data(self, *, object_list=None, **kwargs):
		context = super().get_context_data(**kwargs)
		c_def = self.get_user_context(title="Регистрация")
		return dict(list(context.items()) + list(c_def.items()))#тут формируется объединенный контекст
...
class LoginUser(DataMixin, LoginView):
	...
	def get_success_url(self):#это фукнция для перенаправления после авторизации. Вызывается в случае если пользователь верно ввел логин и пароль, то есть форма прошла валидацию. Также такую настройку можно прописать и в пакете конфигурации setting.py, там нужно прописать константу LOGIN_REDIRECT_URL = '/', в кавычках указываем адрес url, "/" означает главную страницу. Это одно и тоже 
